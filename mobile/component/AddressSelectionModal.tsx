...
const AddressSelectionModal = ({ isProcessing, onClose, onProceed, visible }: AddressSelectionModalProps) => {
        ...
        <Modal visible={visible} animationType='slide' transparent={true} onRequestClose={onClose}>
            <View className="flex-1 bg-black/50 justify-end">
                <View className="bg-background rounded-t-3xl h-1/2">
                    ...
                    {/* Addresses list */}
                    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 50 }} className='flex-1 p-6'>
                        {addressesloading ? (
                            <View className="py-8">
                    ...
                    <View className='p-6 border-t border-surface '>
                        <TouchableOpacity
                            className={`bg-primary rounded-2xl py-5 ${!selectedAddress?"opacity-50":""} `}
                            activeOpacity={0.9}
                            onPress={() => {
                                if (selectedAddress){
