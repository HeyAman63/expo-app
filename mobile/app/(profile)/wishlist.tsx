...
const WishlistScreen = () => {
  ...
  const { wishlist, addToWishlist, isLoading, isError, removeFromWishlist, isRemovingFromWishlist } = useWishlist();
  const { addToCart, isAddingToCart } = useCart();

  const handleRemoveFromWishlist = (productId: string, productName: string) => { 
    Alert.alert("Remove from wishlist",`Remove ${productName} from the wishlist`,[
  ...
  const handleAddToCart =(productId: string, productName: string) => { 
    addToCart({productId,quantity:1},
      {
        onSuccess:()=>Alert.alert("Added to cart",`${productName} added to cart!`),
        onError:(error:any)=>{
          Alert.alert("Error",error?.response?.data?.error|| "Faild to add to cart");
        }
      }
    );
  }
  ...
  return (
    <SafeScreen>
      {/* HEADER */}
      <View className="px-6 pb-5 border-b border-surface py-2 flex-row justify-between mt-2 items-center">
        <View className='flex-row justify-center'>
          <TouchableOpacity onPress={() => router.back()} activeOpacity={0.7} className="mr-4 ">
            <Ionicons name="arrow-back" size={28} color="#FFFFFF" />
          </TouchableOpacity>
          <Text className="text-text-primary text-2xl ml-3 font-bold">Wishlist</Text>
        </View>
                  ...
                  <View className='flex-1 ml-4'>
                    ...
                    {item.stock > 0 ? (
                      <View className="flex-row items-center">
                        ...
                        <Text className="text-green-500 text-sm font-semibold">{item.stock} in stock </Text>
                      </View>
                    ) : (
                      <View className="flex-row items-center">
